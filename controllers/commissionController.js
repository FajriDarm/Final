...
exports.getAllCommissions = async (req, res) => {
    try {
        ...
        res.status(500).render('admin/commissions');
    }
}
...
exports.createCommission = async (req, res) => {
    try {
        ...
        const [result] = await db.query(`
            ...
            VALUES (?, ?, ?, ?, ?, 1)
        `, [eventIdValue, commission_type, commission_value, minStageValue, created_by]);
        ...
        await db.query(`
            ...
            VALUES (?, ?, ?, ?, ?, ?)
        `, [created_by, 'CREATE', 'commission_rule', result.insertId, 'active', description]);
...
exports.updateCommission = async (req, res) => {
    try {
        const { id } = req.params;
        const { commission_type, commission_value, min_stage, is_active } = req.body;
        ...
        if (commission_type === 'percentage' && commission_value && commission_value > 100) {
            return res.status(400).json({ 
                success: false, 
                message: 'Percentage commission cannot exceed 100%' 
            });
...
exports.deleteCommission = async (req, res) => {
    try {
        ...
        await db.query(`
            ...
            (approved_by, action, target_type, target_id, new_status, description)
            VALUES (?, ?, ?, ?, ?, ?)
        `, [deleted_by, 'DELETE', 'commission_rule', id, 'deleted', `Deleted commission rule ${id}`]);
